...
export const Calculate: React.FC<CalculateProps> = props => {
    return (
        ...
        <button onClick={props.resultButton} className="form__submit-button button" type="submit" name="submit" disabled={!props.value}>
          Рассчитать
        </button>
        <button onClick={props.clearButton} className="form__reset-button" name="reset" type="reset" disabled={!props.value}>
